...
if (fs.existsSync(envPath)) {
    try {
        ...
        console.error(chalk.yellow(`⚠️ Failed to load .env file: ${e.message}`));
    }
}
// Default Values
let _usePairingCode = process.env.USE_PAIR_CODE !== undefined ? (process.env.USE_PAIR_CODE === 'true') : true; 
let _phoneNumber = process.env.WHATSAPP_NUMBER || "";      
let _MONGODB_URI = process.env.MONGODB_URI || "";          
let _AuthCode = process.env.AUTH_CODE || "";               
let _Password = process.env.PANEL_PASSWORD || "";  
process.env.HEAVSTAL_API_KEY = global.HT_API_KEY || process.env.HT_API_KEY;

if (fs.existsSync(backupFile)) {
    try {
        const backup = JSON.parse(fs.readFileSync(backupFile, 'utf8'));
        if (backup.usePairingCode !== undefined && process.env.USE_PAIR_CODE === undefined) {
            _usePairingCode = backup.usePairingCode;
        }
        if (backup.phoneNumber && !_phoneNumber) {
            _phoneNumber = backup.phoneNumber;
        }
        if (backup.MONGODB_URI && !_MONGODB_URI) {
            _MONGODB_URI = backup.MONGODB_URI;
        }
        if (backup.AuthCode && !_AuthCode) {
            _AuthCode = backup.AuthCode;
        }
        if (backup.Password && !_Password) {
            _Password = backup.Password;
        }
    } catch (e) {
...
Object.defineProperties(global, {
    ...
    Password: { get: () => _Password, set: (val) => { _Password = val; saveBackup(); } }
});
