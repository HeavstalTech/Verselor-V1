...
export default [
    ...
    },
    {
        ...
        execute: async (HeavstalTech, m, context) => {
            ...
            let pdfBuffer = null
            try {
                pdfBuffer = await HeavstalTech.downloadAndSaveMediaMessage(quoted);
                ...
                if (pdfBuffer && fs.existsSync(pdfBuffer)) fs.unlinkSync(pdfBuffer);
            }
        }
    },
    {
...
> 𝐂𝐫𝐞𝐝𝐢𝐭:- 𝐇𝐄𝐀𝐕𝐒𝐓𝐀𝐋 𝐓𝐄𝐂𝐇`,
    ...
    },
    {
        ...
        execute: async (HeavstalTech, m, context) => {
            ...
            if (!m.quoted || m.quoted.mtype !== 'stickerMessage') {
                return Vreply(`_Please reply to a STICKER with ${prefix + command} <command>_`);
            }
            if (!text) return Vreply(`_Provide a command also.._`);

            const f = text.trim().split(/\s+/)[0];
            const hash = m.quoted.fileSha256 || m.quoted.msg?.fileSha256 || null;
            if (!hash) return Vreply("*Couldn't get sticker hash*");
            const stickerHash = Buffer.from(hash).toString('hex');
            global.db.data.sticker_cmd[stickerHash] = text;
    ...
    },
    {
        ...
        ownerOnly: true,
        execute: async (HeavstalTech, m, context) => {
            const { Vreply, prefix, command } = context;
            if (!m.quoted || m.quoted.mtype !== 'stickerMessage') {
                return Vreply(`_Reply to a bound sticker with ${prefix + command} to delete it._`);
            }
            const rawHash = m.quoted.fileSha256 || m.quoted.msg?.fileSha256;
            if (!rawHash) return Vreply(`_Hash not found. Try a newer sticker._`);    
            const stickerHash = Buffer.from(rawHash).toString("hex");
            if (!global.db.data.sticker_cmd[stickerHash]) {
                return Vreply(`_This sticker has no command bound to it in the database._`);
            }
            const oldCmd = global.db.data.sticker_cmd[stickerHash];
            delete global.db.data.sticker_cmd[stickerHash];
            await global.db.write();
            Vreply(`🗑️ *Command Deleted:* From to *${oldCmd}*`);
        }
    },
    {
        name: 'listcmd',
        ...
        execute: async (HeavstalTech, m, context) => {
            ...
            let responseText = `*📋 STICKER COMMANDS LIST*\n\n`;
            entries.forEach(([hash, cmd], i) => {
                responseText += `*${i + 1}.* 🕹️ *Command:* ${cmd}\n`;
                responseText += `   🆔 *Hash:* \`${hash.slice(0, 15)}...\`\n\n`;
            });       
            context.Vreply(responseText.trim() + `\n\n_Total: ${entries.length} commands_`);
        }
    },
    {
        name: 'time',
