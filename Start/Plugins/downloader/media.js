...
import { someResponseCollector, apiConfig } from "#System/Data1.js";
...
export default [
    {
        ...
        execute: async (HeavstalTech, m, context) => {
            ...
            try {
                ...
                try {
                    const anu = await downloader.ytmp3(res.url, apiConfig);
                    if (anu && anu.url) {
                      /*  let urlMp3 = anu.url;
                        await HeavstalTech.sendMessage(m.chat, {
                            audio: { url: urlMp3 }, 
            ...
            try {
                ...
                try {
                    ...
                    if (choice === '1' || choice.toLowerCase() === 'audio') {
                        ...
                        const mp3Link = await downloader.ytmp3(videoResult.url, apiConfig);
                        ...
                        const mp4Link = await downloader.ytmp4(videoResult.url, apiConfig);
                        ...
                        const mp3Doc = await downloader.ytmp3(videoResult.url, apiConfig);
                        await HeavstalTech.sendMessage(m.chat, { 
    ...
    },
    {
        ...
        execute: async (HeavstalTech, m, context) => {
    const { text, Vreply, mess } = context;
    if (!text) return Vreply(`Provide a link.`);
    Vreply(mess.wait);

    try {
        const data = await downloader.tiktok(text, apiConfig);
        await HeavstalTech.sendMessage(m.chat, {
        ...
        execute: async (HeavstalTech, m, context) => {
            ...
            try {
                ...
                for (let video of result) {
                    await HeavstalTech.sendFile(m.chat, video.url, '𝐕𝐄𝐑𝐒𝐄𝐋𝐎𝐑 𝐕𝟏 ²⁶.', m);
                }
            } catch (err) {
                console.error(err);
