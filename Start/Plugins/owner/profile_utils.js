...
export default [
    ...
    },
  {
        ...
        aliases: ['save'],
        category: 'owner',
        description: 'Saves the replied media and forwards it to the Owner\'s DM using disk-streaming.',
        usage: '%prefix%save <reply media>',
        ownerOnly: true,
        execute: async (HeavstalTech, m, context) => {
            const { Vreply, BotNum, command, mess } = context;
            if (!m.quoted) return Vreply("⚠️ Please reply to a media message (image, video, audio, sticker, or document).")
            await HeavstalTech.sendMessage(m.chat, { react: { text: '💾', key: m.key } });
            let tempFilePath = null;
            try {
                tempFilePath = await HeavstalTech.downloadAndSaveMediaMessage(m.quoted, `save_stream_${Date.now()}`);
                if (!fs.existsSync(tempFilePath)) throw new Error("Failed to write media to disk.")
                const mime = m.quoted.mimetype || m.quoted.msg?.mimetype || '';
                const caption = m.quoted.text || m.quoted.caption || '';
                let messageOptions = {};
                if (/image/.test(mime)) {
                    messageOptions = { image: { url: tempFilePath }, caption };
                } else if (/video/.test(mime)) {
                    messageOptions = { video: { url: tempFilePath }, caption };
                } else if (/audio/.test(mime)) {
                    messageOptions = { audio: { url: tempFilePath }, mimetype: mime, ptt: mime.includes('ogg') };
                } else if (/sticker/.test(mime)) {
                    messageOptions = { sticker: { url: tempFilePath } };
                } else {
                    messageOptions = { 
                        document: { url: tempFilePath }, 
                        mimetype: mime, 
                        fileName: m.quoted.fileName || `saved_file_${Date.now()}` 
                    };
                }
                await HeavstalTech.sendMessage(BotNum, messageOptions);
                
                Vreply("✅ Media successfully saved and forwarded to your DMs.");

            } catch (e) {
                console.error(`[SAVE COMMAND ERROR]:`, e);
                Vreply(`❌ Failed to save media.\n_Error: ${e.message}_`);
            } finally {
    ...
    },
    {
        ...
        execute: async (HeavstalTech, m, context) => {
            ...
            try {
                if (/image/.test(mime)) {
                    imgSwPath = await HeavstalTech.downloadAndSaveMediaMessage(quoted, "status_img");
                    buffer = fs.readFileSync(imgSwPath);
                    ...
                    VidSwPath = await HeavstalTech.downloadAndSaveMediaMessage(quoted, "status_vid");
                    buffer = fs.readFileSync(VidSwPath);
                    await HeavstalTech.sendMessage(
                        "status@broadcast",
                    ...
                    VnSwPath = await HeavstalTech.downloadAndSaveMediaMessage(quoted, "status_aud");
                    buffer = fs.readFileSync(VnSwPath);
                    await HeavstalTech.sendMessage(
                        "status@broadcast",
                ...
                let PathToDelete = imgSwPath || VidSwPath || VnSwPath
                if (PathToDelete && fs.existsSync(PathToDelete)) fs.unlinkSync(PathToDelete);
                PathToDelete = null
                buffer = null
            }
        }
    ...
    },
    {
        ...
        execute: async (HeavstalTech, m, context) => {
            ...
            let targetJid = m.quoted ? (m.quoted.sender || m.chat) : (m.mentionedJid?.[0] || m.chat); // unexpected strict mode reserved word - fixed
            let ppUrl;
