...
export default [
    ...
    },
   {
    ...
    {
    name: 'gcstatuslink',
    aliases: ['gsl', 'upswlink'],
    category: 'group',
    description: 'Post a group status using an invite link (even if not in the group).',
    usage: '%prefix%gcstatuslink <link> | <text> (or reply to media)',
    ownerOnly: true,
    execute: async (HeavstalTech, m, context) => {
        const { text, quoted, mime, Vreply, prefix, command } = context;
        if (!text) return Vreply(`*Usage:* ${prefix + command} <link> | <message>\n*Example:* ${prefix + command} https://chat.whatsapp.com/ID | Hello world`);
        let linkPart, statusText;
        if (text.includes('|')) {
            const parts = text.split('|');
            linkPart = parts[0].trim();
            statusText = parts.slice(1).join('|').trim();
        } else {
            linkPart = text.trim();
            statusText = '';
        }
        let coded = linkPart.split("https://chat.whatsapp.com/")[1];
        if (coded) coded = coded.split("?")[0].trim();
        if (!coded) return Vreply(`❌ Invalid WhatsApp link provided.`);
        await HeavstalTech.sendMessage(m.chat, { react: { text: '🔍', key: m.key } });        
        try {
            const res = await HeavstalTech.query({
                tag: "iq",
            ...
            if (!res || !res.content || !res.content[0]) {
                return Vreply("❌ Could not fetch group info. Link may be expired.");
            }

            const targetJid = res.content[0].attrs.id + "@g.us";
            const groupName = res.content[0].attrs.subject || "Target Group";
            ...
            if (isImage || isVideo || isAudio) {
                tempFilePath = await HeavstalTech.downloadAndSaveMediaMessage(quoted, `link_status_${Date.now()}`);
                
                let statusOptions = {};
                if (isImage) {
                    statusOptions = { image: { url: tempFilePath }, caption: statusText };
                } else if (isVideo) {
                    statusOptions = { video: { url: tempFilePath }, caption: statusText };
                } else if (isAudio) {
                    statusOptions = { audio: { url: tempFilePath }, mimetype: 'audio/mp4' };
                }
                await HeavstalTech.sendGroupStatus(targetJid, statusOptions, { quoted: m });
            } else {
                if (!statusText) return Vreply("❌ Please provide text after the | symbol.");
                await HeavstalTech.sendGroupStatus(targetJid, { text: statusText }, { quoted: m });
            }

            await Vreply(`✅ Status successfully posted to:\n*${groupName}*\n(${targetJid})`);
            await HeavstalTech.sendMessage(m.chat, { react: { text: '✅', key: m.key } });
            if (tempFilePath && fs.existsSync(tempFilePath)) {
                fs.unlinkSync(tempFilePath);
            }
        } catch (e) {
            console.error("[GC LINK STATUS ERROR]", e);
