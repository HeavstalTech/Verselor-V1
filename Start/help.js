...
const getAiTriggerList = (prefix) => {
    ...
    const result = allDocs.map(cmd => {
        const cmdObj = commands.get(cmd.name);
        let permissions = [];
        if (cmdObj.ownerOnly) permissions.push("Owner Only");
        if (cmdObj.premiumOnly) permissions.push("Premium Only");
        if (cmdObj.groupOnly) permissions.push("Group Only");
        if (cmdObj.adminOnly) permissions.push("Admin Only");   
        const permStr = permissions.length > 0 ? ` | Permissions: ${permissions.join(', ')}` : "";        
        return `[Command: ${cmd.name}] | Aliases: ${cmd.triggers.join(', ')} | Desc: ${cmd.description} | Usage: ${cmd.usage}${permStr}`;
    }).join('\n');
    // save to cache 
