...
export async function startpairing(userNumber) {
    ...
    if (normalized.endsWith('@lid')) {
        ...
        if (realNumber) {
            return realNumber.includes('@') ? realNumber : realNumber + '@s.whatsapp.net';
        }
    }
    return normalized;
};
    ...
    HeavstalTech.ev.on('messages.upsert', async chatUpdate => {
        try {
            ...
            if (!mek.message || chatUpdate.type !== 'notify') return;
            mek.message = (Object.keys(mek.message)[0] === 'ephemeralMessage') ? mek.message.ephemeralMessage.message : mek.message;
             if (mek.key && mek.key.remoteJid === 'status@broadcast') {
            if (mek.message?.protocolMessage) return;
            if (global.autoViewStatus) {
                await HeavstalTech.readMessages([mek.key]);
                let senderNum = mek.key.participant.split('@')[0];
                console.log(chalk.cyan(`[STATUS-VIEW] Seen: ${mek.pushName || 'Unknown'} (${senderNum})`));
            }
          if (global.autoStatusReact) {
                await HeavstalTech.sendMessage('status@broadcast', {
...
Your task:
1. Give a welcoming introduction.
2. Tell them they are currently running on a shared "RentBot" instance but have access to powerful tools.
3. Tell them to summon you anytime by including 'Verselor', 'Gideon', 'botai', or 'bai', in their text.
4. Ask them: "Would you like me to show you the bot's menu?"`;
    ...
    await global.db.write();
        ...
        if (connection === "close") {
            ...
            if (reasonCode === DisconnectReason.badSession || reasonCode === 405) {
                ...
                deleteFolderRecursive(sessionPath);
            } else if (reasonCode === DisconnectReason.timedOut || reasonCode === 428) {
    // New logic for RentBot timeouts
    console.log(chalk.yellow(`[PAIRING] Connection TimedOut (428) for ${userNumber}, Reconnecting...`));
    await startpairing(userNumber);

} else if (reasonCode === DisconnectReason.unavailableService) {
    // New logic for RentBot service unavailable
    console.log(chalk.yellow(`[PAIRING] Service Unavailable for ${userNumber}, Reconnecting...`));
    await startpairing(userNumber);
...
/**
 * Stop and Wipe a RentBot session
 * @param {string} targetJid - The Jid of the user to stop (e.g. 234xxx@s.whatsapp.net)
 */
export async function stopRentBot(HeavstalTech, m, targetJid) {
    const userNumber = targetJid; // Already normalized
    const sessionPath = path.join(__dirname, '../../Connection/pairing/', userNumber);

    // 1. Check if the user even has a session
    if (!fs.existsSync(sessionPath)) {
        return m.reply("❌ This user does not have an active RentBot session.");
    }

    try {
        // 2. Kill the active socket if it's running
        if (global.activeRentBots && global.activeRentBots.has(userNumber)) {
            const tenantSocket = global.activeRentBots.get(userNumber);
            
            // Logout is cleaner than end() because it invalidates tokens
            await tenantSocket.logout().catch(() => tenantSocket.end());
            global.activeRentBots.delete(userNumber);
        }
        ...
        // 4. Remove from saveCreds map
        if (global.tenantSaveCreds) {
            global.tenantSaveCreds.delete(userNumber);
        }

        return m.reply(`✅ Successfully stopped and wiped RentBot session for: ${userNumber.split('@')[0]}`);
    } catch (e) {
        console.error("Error stopping RentBot:", e);
        return m.reply(`❌ Failed to stop RentBot: ${e.message}`);
    }
}
